...
### - Análise de Eventos de Erro e Fontes de Variabilidade no Experimento da Catapulta
  ...
  A variabilidade observada nos arremessos da catapulta, quantificada por um
Coeficiente de Variação de 6.59% e pela presença de outliers, sinaliza a necessidade
de refinamentos metodológicos. As principais fontes de erro podem ser categorizadas
e abordadas da seguinte forma:  
 ...
 * A análise estatística do experimento da catapulta revelou uma distribuição de arremessos caracterizada por uma média de 332.90 cm e um desvio padrão de 21.93 cm. A assimetria negativa (-1.3815) e a leptocurtose (3.4561) são indicadores cruciais da dinâmica do sistema. A assimetria sugere que, embora a maioria dosarremessos tenda a distâncias mais longas, a ocorrência de alguns arremessos significativamente mais curtos (outliers) desloca a cauda da distribuição para a esquerda. Este fenômeno pode ser atribuído a falhas pontuais na liberação do projétil ou a interações imprevistas com o mecanismo da catapulta, resultando em perda de energia cinética.

 * A leptocurtose, por sua vez, indica uma maior concentração de dados em torno da
média (pico mais agudo) e, simultaneamente, a presença de caudas mais pesadas,
confirmando a existência de valores extremos. Isso implica que o sistema, apesar de
uma tendência central, é propenso a produzir resultados tanto muito curtos quanto
muito longos, o que pode ser explicado pela sensibilidade do sistema a pequenas
variações nas condições iniciais de lançamento (e.g., ângulo de saída, tensão do
elástico) e pela natureza estocástica de interações com o ambiente. A identificação de
outliers, particularmente os arremessos de 257.9 cm a 302.5 cm e de 365.0 cm a 369.0
cm, corrobora essa interpretação, destacando a necessidade de investigar as causas
específicas desses eventos extremos.

 * Em suma, o experimento demonstra a complexidade inerente à modelagem de
sistemas físicos sob condições semi-controladas. A variabilidade observada não é
meramente um ruído, mas uma manifestação das interações dinâmicas entre o
equipamento, o operador e o ambiente. 
