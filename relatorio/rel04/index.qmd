...
ggplot(dados, aes(x = falhas, y = probabilidade)) +
  ...
  theme_minimal()
```
* **O ggplot2serve para criar gráficos através de uma lógica de camadas.**
...
$a$: Limite mínimo do intervalo (ex: tensão mínima de operação).
$b$: Limite máximo do intervalo (ex: tensão máxima de operação).
Média (Esperança): $E[X] = \frac{a + b}{2}$
Variância: $Var(X) = \frac{(b - a)^2}{12}$

* **Por Que é Relevante na Engenharia Mecatrônica?**

-Resolução de Sensores: Se um sensor de posição tem uma resolução de 0,01 mm, o erro de leitura está distribuído uniformemente entre $-0,005$ e $+0,005$;
...
#### Exemplo Aplicado: Posicionamento de um Atuador Linear.
...
$MTTF$ (Mean Time To Failure): O Tempo Médio Até a Falha. É o valor esperado (a média) da distribuição. É inversamente proporcional à taxa de falha:$$\mu = MTTF = \frac{1}{\lambda}$$
...
### Estudo de Caso Prático: O desenvolvimento do painel de controle (CLP - Controlador Lógico Programável)
...
print(paste("Probabilidade de falha:", round(prob_falha * 100, 2), "%"))

# Qual a confiabilidade (probabilidade de sobreviver)? R(t)
...
print(paste("Confiabilidade:", round(confiabilidade * 100, 2), "%"))

# Simular o tempo de falha de 100 sensores (Monte Carlo)
